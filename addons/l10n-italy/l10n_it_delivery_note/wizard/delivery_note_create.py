...
class StockDeliveryNoteCreateWizard(models.TransientModel):
    ...
    def confirm(self):
        ...
        order_lines = sale_order_id.order_line
        total_weight = 0
        total_volume = 0
        total_packages = 0

        for order_line in order_lines:
            total_packages += order_line.product_packaging_qty 
            total_weight += order_line.product_uom_qty * order_line.product_template_id.weight
            total_volume += order_line.product_uom_qty * order_line.product_template_id.volume
        ...
        if sale_order_id:
            ...
            for invoice in sale_order_id.invoice_ids:
                if type(invoice) == type(int()):
                    invoice_ids_list.append(invoice)
                else:
                    invoice_ids_list.append(invoice.id)

            sale_order_id._assign_delivery_notes_invoices(invoice_ids_list)
