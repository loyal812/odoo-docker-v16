...
class MigrateL10nItDdt(EasyCommand):
    ...
    def _map_ref(self, map_dict, old_ext_id, new_ext_id):
        old_record = self.env.ref("l10n_it_stock_ddt.{}".format(old_ext_id))
    ...
    def check_database_integrity(self):
        ...
        if not old_module or old_module[1] != "installed":
            raise UserError(
                _(
                    "Module `l10n_it_stock_ddt` isn't installed on this database. "
                    "You don't need to run this command."
                )
            )

        old_sequence = self.env.ref("l10n_it_stock_ddt.seq_ddt")
    ...
    def migrate_document_types(self):
        ...
        old_type = self.env.ref("l10n_it_stock_ddt.ddt_type_ddt")
        ...
        self.env.cr.execute(
            ...
            WHERE "module" = 'l10n_it_stock_ddt' AND "name" = 'seq_ddt';
        """
        )
